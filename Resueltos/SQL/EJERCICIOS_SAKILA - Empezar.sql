...
SELECT TITLE FROM FILM ORDER BY LENGTH ASC LIMIT 1;
...
SELECT COUNT(*) AS NUMERO_PELICULAS_E_ELIMINADAS
	FROM FILM WHERE SPECIAL_FEATURES LIKE "%DELETED SCENES%";


# EJ7 ¿QUÉ PELÍCULAS TIENEN "ALIEN" EN SU NOMBRE? (MUESTRA POR ORDEN ALFABÉTICO)

SELECT TITLE FROM FILM WHERE TITLE LIKE "%ALIEN%" ORDER BY TITLE ASC;

# EJ8 ¿CUÁL ES LA PELÍCULA MÁS LARGA QUE PODEMOS VER POR MENOS DE 1$ (RENTAL_RATE)?

SELECT TITLE FROM FILM WHERE RENTAL_RATE<1.01 ORDER BY LENGTH DESC LIMIT 1;
...
SELECT
	COUNT(TITLE) AS PELICULAS_TOTALES,
	SUM(LENGTH)/60/24 AS DIAS_VISUALIZACION,
	SUM(LENGTH)/60/24/365 AS ANOS_VISUALIZACION
    FROM FILM;
...
WHERE LENGTH > 100 AND RATING IN ("PG","R","G");

# PT7 - EJ3.- ¿CUÁL ES LA PELÍCULA O PELÍCULAS CON MAYOR DURACIÓN DE LAS QUE CONTIENEN TRAILERS COMO EXTRAS?

SELECT TITLE,LENGTH FROM FILM
WHERE LENGTH = (SELECT MAX(LENGTH) FROM FILM WHERE SPECIAL_FEATURES LIKE "%TRAILERS%" AND TITLE LIKE "A%")
AND SPECIAL_FEATURES LIKE "%TRAILERS%" AND TITLE LIKE "A%";


SELECT MAX(LENGTH) FROM FILM WHERE SPECIAL_FEATURES LIKE "%TRAILERS%";

SELECT TITLE FROM FILM 
WHERE LENGTH = (185) AND SPECIAL_FEATURES LIKE "%TRAILERS%";
;


# PT7 - EJ4.- ¿QUÉ PELÍCULAS ESTÁN PROTAGONIZADAS POR UN ASTRONAUTA Y UN ADMINSTRADOR DE BASES DE DATOS (DESCRIPTION)?

SELECT TITLE FROM FILM WHERE DESCRIPTION LIKE "%ASTRONAUT%" AND DESCRIPTION LIKE "%ADMIN%";
...
SELECT SUM(LENGTH)/60 AS LUKE
FROM FILM WHERE FILM_ID IN (SELECT FILM_ID FROM FILM_CATEGORY WHERE CATEGORY_ID = 6);
...
SELECT AVG(LENGTH),RATING FROM FILM WHERE SPECIAL_FEATURES NOT LIKE "%DELETED SCENES%" GROUP BY RATING;
...
SELECT CEILING(SUM(REPLACEMENT_COST)) FROM FILM WHERE FILM_ID IN(SELECT FILM_ID FROM FILM_ACTOR WHERE ACTOR_ID IN(SELECT ACTOR_ID FROM ACTOR WHERE FIRST_NAME like "JENNIFER" AND LAST_NAME like "DAVIS"));
...
SELECT REVERSE(LOWER(CONCAT(LEFT(FIRST_NAME,3),RIGHT(LAST_NAME,3)))) AS NOMBRE
...
# EJ22 SELECCIONA EL TÍTULO DE LAS PELÍCULAS, UNA COLUMNA QUE MUESTRE SI DURA MÁS O MENOS DE 2H, OTRA COLUMNAQUE MUESTRE SI TIENE O NO ESCENAS ELIMINADAS Y OTRA QUE MUESTRE SI TIENE O NO TRAILERS (COLUMNAS BOOLEANAS).

SELECT	TITLE,
		BOOLEAN(LENGTH>120) AS MAS_2_HORAS,
        BOOLEAN(SPECIAL_FEATURES LIKE "%DELETED SCENES%") AS ESCENAS_ELIMINADAS,
        BOOLEAN(SPECIAL_FEATURES LIKE "%TRAILER%")
        
        FROM FILM;
        

# EJ23 SELECCIONA EL TÍTULO DE LAS PELÍCULAS Y UNA QUE MUESTRE SI LA PELÍCULA ES APTA PARA TODOS LOS PÚBLICOS (G, PG Y PG-13)
